...
type Step = 1 | 2 | 3 | 4;
type MockFile = { id: number; name: string; size: string; type: string; captureSource: "upload" | "camera"; file?: File };
const steps = [
...
export function SignupFlow({ embedded = false, sandboxUserId }: { embedded?: boolean; sandboxUserId?: string }) {
  ...
  function addFiles(event: React.ChangeEvent<HTMLInputElement>, captureSource: "upload" | "camera" = "upload") {
    const selected = Array.from(event.target.files ?? []).map((file, index) => ({ id: Date.now() + index, name: file.name, size: `${(file.size / 1024 / 1024).toFixed(1)} MB`, type: file.type.split("/").pop()?.toUpperCase() ?? "FILE", captureSource, file }));
